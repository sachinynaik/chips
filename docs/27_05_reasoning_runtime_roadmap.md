...
|---|---|---|---|
| **0** | Anti-regression constraint memory (`cortex_constraints`): scoped, manual add/retire, force-injected into `hard_constraints`/`forbidden_edits` as a dynamic policy layer beside the static `PolicyLoader` | Designed & locked | — |
| **1** | Evidence-ranked hypotheses: stable evidence IDs, `EvidenceBundle`, `Hypothesis` contract, deterministic coverage−contradiction ranking, write-back review queue | **Locked contract; implementing next** ([doc](./27_05_phase1_evidence_hypotheses_contract.md)) | Phase 0 |
